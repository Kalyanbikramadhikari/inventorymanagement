...
const CustomersTopCard = ({name, amount}) => {
  return (
    <div className='w-2/4 flex gap-5  border border-inputField-border rounded-borderRadius  p-6'>
        <div className="flex ">
            ...
            <div className="flex flex-col border-r pr-6 border-inputField-border">
                <span className='text-subHeading text-[#7A8299] font-medium'>{name}</span>
                <h1 className='text-[32px]'>{amount}</h1>
            </div>

            <CustomerStats topfield="false"/>
    </div>
  )
}

export default CustomersTopCard